...
import { Scissors, MapPin, Clock, Smartphone, Crown } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const [{ data: svcs, error: svcsErr }, { data: plansData }] = await Promise.all([
          supabase
            .from("services")
          ...
          supabase
            ...
            .select("id, nome, preco_mensal, cortes_inclusos, descricao")
            .eq("tenant_id", TENANT_ID)
            .eq("ativo", true)
            .order("preco_mensal"),
        ]);
  ...
  const categoriasPresentes = ["Todos", ...Array.from(new Set(services.map(s => s.categoria || "Outros")))];
  ...
  return (
    <main className="relative min-h-screen w-full flex flex-col items-center overflow-x-hidden bg-black">
      ...
      <section className="relative z-20 min-h-screen w-full flex flex-col items-center justify-between p-6 pb-12 overflow-hidden">
        ...
        <header className="mt-12 text-center">
          <h2 className="text-white font-black uppercase italic tracking-tighter text-2xl">
            Barbearia{" "}
            <span className="text-orange-600 drop-shadow-[0_0_8px_rgba(249,115,22,0.5)]">BNB</span>
          </h2>
      ...
      {/* ✅ PLANOS DE ASSINATURA (se existirem) */}
      {plans.length > 0 && (
        <section className="relative z-20 w-full max-w-2xl px-4 py-16">
          <div className="text-center mb-10">
            <h3 className="text-orange-500 font-bold tracking-[0.4em] uppercase text-[10px] mb-2 flex items-center justify-center gap-2">
              <Crown size={12} /> Assinatura
            </h3>
            <h2 className="text-4xl font-black uppercase italic text-white tracking-tighter">
              Planos <span className="text-orange-600">Mensais</span>
            </h2>
            <p className="text-zinc-500 text-xs mt-3 uppercase tracking-widest">
              Cortes ilimitados com preço fixo todo mês
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {plans.map(plan => (
              <div
                key={plan.id}
                className="bg-zinc-900/50 backdrop-blur-3xl border border-orange-600/20 rounded-[2.5rem] p-8 shadow-2xl shadow-orange-600/5"
              >
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-white font-black uppercase italic text-xl tracking-tight">{plan.nome}</p>
                    <p className="text-zinc-500 text-[9px] uppercase tracking-widest mt-1">{plan.descricao}</p>
                    <p className="text-orange-500 text-[10px] font-black uppercase mt-3 flex items-center gap-1">
                      <Scissors size={10} /> {plan.cortes_inclusos} cortes por mês
                    </p>
                  </div>
                  <div className="text-right">
                    <span className="text-orange-600 font-black text-3xl italic">
                      R$ {Number(plan.preco_mensal).toFixed(0)}
                    </span>
                    <p className="text-zinc-600 text-[8px] uppercase font-bold">/mês</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* ✅ TABELA DE PREÇOS COM CATEGORIAS */}
      <section id="precos" className="relative z-20 w-full max-w-2xl px-4 py-24">
        ...
        <div className="bg-zinc-900/50 backdrop-blur-3xl border border-white/5 rounded-[3rem] p-8 md:p-12 shadow-2xl">
          {loading ? (
              ...
              <div className="flex gap-2 flex-wrap mb-8">
                {categoriasPresentes.map(cat => (
                  <button
                    ...
                    className={`text-[8px] font-black uppercase tracking-[0.25em] px-3 py-1.5 rounded-full border transition-all ${
                      categoriaAtiva === cat
                        ? "bg-orange-600 text-white border-orange-600"
                        : "border-zinc-700 text-zinc-500 hover:border-orange-600/40"
                    }`}
                  >
                    {cat}
                  </button>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-zinc-500 text-[9px] font-bold uppercase mt-1 tracking-widest">
                          {service.categoria || "Procedimento Premium"}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
      ...
      <section className="relative z-20 w-full max-w-5xl px-4 pb-20">
        <div className="bg-zinc-900/40 backdrop-blur-3xl border border-white/5 rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col lg:flex-row">
          <div className="flex-1 p-10 md:p-14 space-y-10">
            <div>
              ...
              <p className="text-zinc-500 text-[9px] font-bold uppercase tracking-[0.3em]">Estilo & Excelência</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div>
                ...
                <p className="text-white text-sm font-medium leading-relaxed uppercase">
                  Rua Adilia Cajazeiras, 77<br />Centro, Banabuiú - CE
                </p>
              </div>
              <div>
                <h4 className="text-orange-600 text-[10px] font-black uppercase tracking-widest mb-3 flex items-center gap-2">
                ...
                <p className="text-white text-sm font-medium uppercase">
                  Seg - Sáb  09h às 21h<br />Domingo Fechado
                </p>
              </div>
            </div>

            <div className="pt-6 border-t border-white/5">
              <a href="tel:+5588997018660" className="text-white text-xl font-black italic tracking-tight underline decoration-orange-600/30 underline-offset-8 flex items-center gap-2 transition-all hover:text-orange-500">
                <Smartphone size={20} className="text-orange-600" />
                <span>(88) 997018660   Renan Jhamesson</span>
              </a>
            </div>
            <div className="pt-6 border-t border-white/5">
              <a href="tel:+5588998052628" className="text-white text-xl font-black italic tracking-tight underline decoration-orange-600/30 underline-offset-8 flex items-center gap-2 transition-all hover:text-orange-500">
                <Smartphone size={20} className="text-orange-600" />
        ...
        <div className="mt-8 text-center">
          <p className="text-zinc-600 text-[8px] font-bold uppercase tracking-[0.4em]">
            <strong>© 2026 Barbearia BNB • Desenvolvido por InputTech</strong>
          </p>
        </div>
      </section>
    </main>
