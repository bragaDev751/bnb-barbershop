...
import Image from "next/image" // 1. Importado para performance
...
export default async function Barbeiros({ searchParams }: PageProps) {
  ...
  // Busca os barbeiros de forma eficiente
  const { data: barbers, error } = await supabase
    .from("barbers")
    .select("*")
    .order('name', { ascending: true })

  // 2. Tratamento de erro robusto no Server Component
  if (error) {
    return (
  ...
  return (
    <div className="relative min-h-screen pb-20 px-6 bg-black">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute top-[20%] left-[-10%] w-[400px] h-[400px] bg-orange-500/5 blur-[100px] rounded-full" />
        <div className="absolute bottom-[10%] right-[-5%] w-[300px] h-[300px] bg-orange-500/10 blur-[120px] rounded-full opacity-50" />
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {barbers && barbers.length > 0 ? (
            barbers.map((barber) => (
              <Link
                key={barber.id}
                // 3. Garantindo que o serviceId continue fluindo na URL
                href={`/data?service=${serviceId}&barber=${barber.id}`}
                className="group relative flex items-center gap-6 p-6 bg-zinc-900/40 backdrop-blur-2xl border border-white/5 rounded-[2.5rem] hover:border-orange-500/40 hover:bg-zinc-900/60 transition-all duration-500 shadow-2xl active:scale-95"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-orange-500/0 to-orange-500/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-[2.5rem]" />

                <div className="relative z-10">
                  <div className="w-24 h-24 md:w-28 md:h-28 rounded-full overflow-hidden border-2 border-zinc-800 group-hover:border-orange-500 transition-all duration-500 ring-4 ring-transparent group-hover:ring-orange-500/20 relative">
                    {/* 4. Uso do Next Image para carregamento rápido (WebP) */}
                    <Image 
                  ...
                  <div className="absolute bottom-2 right-2 w-5 h-5 bg-orange-500 border-[4px] border-zinc-900 rounded-full shadow-[0_0_10px_rgba(249,115,22,0.8)]" />
                </div>
                ...
                <div className="flex-1 relative z-10">
                  ...
                  <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter group-hover:text-orange-500 transition-colors leading-tight">
                    {barber.name}
                  </h2>
                ...
                <div className="hidden sm:flex w-12 h-12 rounded-2xl border border-white/5 items-center justify-center text-zinc-700 group-hover:border-orange-500 group-hover:text-orange-500 group-hover:bg-orange-500/10 transition-all duration-500">
